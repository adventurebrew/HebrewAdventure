...
def strings_import(rooms_to_recompile):
    ...
    filenames = sorted(list(set([entry[scripts_strings_keys['filename']] for entry in texts])))
    for filename in filenames:
        entries = [entry for entry in texts if entry[scripts_strings_keys['filename']] == filename]

        if set([entry[scripts_strings_keys['translation']] for entry in entries]) == {''}:
        ...
        for entry in entries:
            if entry[scripts_strings_keys['translation']]:
                script = check_replace(script, entry[scripts_strings_keys['original']], entry[scripts_strings_keys['translation']])
