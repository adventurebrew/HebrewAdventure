...
!define UNINSTALLER_NAME "QFG1_heb_uninstaller.exe"
...
Name "הרצון לעוצמה 1: אז אתה רוצה להיות גיבור"

OutFile "qfg1-hebrew-installer.exe"
...
• יש ברשותך עותק תקין של QFG1 בגרסה המחודשת - VGA (ניתן לקנות ב GoG)$\r$\n \
• מותקנת גרסת Daily של ScummVM" ;" ; gvim get's confused without that extra "

!define MUI_FINISHPAGE_TEXT  "ההתקנה הושלמה בהצלחה.$\r$\n$\r$\n \
• כעת ניתן להוסיף את המשחק ל ScummVM, והוא יזוהה כ QFG1 בעברית.$\r$\n \
...
DirText "בחר את התיקייה שקבצי QFG1 נמצאים בה"
...
Section "Update file"
    ...
    IfFileExists $INSTDIR\PATCHES 0 +3
        MessageBox MB_ICONSTOP "נראה כי זו לא התקנה נקיה של QFG1! יש להתחיל מהתקנה נקיה"
		abort
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QFG1_Hebrew" \
                     "DisplayName" "qfg1 Hebrew translation"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QFG1_Hebrew" \
...
Section "Uninstall"
    ...
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\QFG1_Hebrew"
SectionEnd
