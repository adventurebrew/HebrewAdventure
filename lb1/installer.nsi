...
!define UNINSTALLER_NAME "LB1_heb_uninstaller.exe"
...
Name "עזבונו של הקולונל"

OutFile "lb1-hebrew-installer.exe"
...
• יש ברשותך עותק תקין של עזבונו של הקולונל (ניתן לקנות ב GoG)$\r$\n \
• מותקנת גרסת Daily של ScummVM" ;" ; gvim get's confused without that extra "

!define MUI_FINISHPAGE_TEXT  "ההתקנה הושלמה בהצלחה.$\r$\n$\r$\n \
• כעת ניתן להוסיף את המשחק ל ScummVM, והוא יזוהה כ LB1 בעברית.$\r$\n \
...
DirText "בחר את התיקייה שקבצי LB1 נמצאים בה"
...
Section "Update file"
    ...
    IfFileExists $INSTDIR\PATCHES\0.fon 0 +3
        MessageBox MB_ICONSTOP "נראה כי זו לא התקנה נקיה של LB1! יש להתחיל מהתקנה נקיה"
		abort		
    ...
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LB1_Hebrew" \
                     "DisplayName" "LB1 Hebrew translation"
    WriteRegStr HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LB1_Hebrew" \
...
Section "Uninstall"
    ...
    DeleteRegKey HKLM "Software\Microsoft\Windows\CurrentVersion\Uninstall\LB1_Hebrew"
SectionEnd
