...
(instance myStab of Script
	...
	(method (doit)
		...
		(switch local61
			(3
				(if (not (Drip2 cycler?)) (Drip2 show: setCycle: Fwd))
			)
			(5
				(if (not (Drip1 cycler?))
		...
		(if
		(and (== (myMusic prevSignal?) 10) (== state 5))
			(= cycles 1)
		)
	)
	...
	(method (changeState newState)
		(switch (= state newState)
			...
			(4
				(if (!= (myMusic prevSignal?) -1) (= state 3))
				(= cycles 1)
			)
			(5
			...
			(10
				(Glop2 cel: 0)
				(cond 
					(global223 (if (< local61 9) (++ local61) else (= local61 9)))
					((< local61 7) (++ local61) (++ local61) (++ local61))
					(else (= local61 9))
				)
				(Blood cel: local61)
				(if (== (myMusic prevSignal?) -1)
