...
(class SRDialog kindof Dialog
	...
	(method (init theComment names nums)
		...
		(= numGames (GetSaveFiles (gGame name?) names nums))
		(if (== numGames -1)
			(return FALSE)
		)
	...
	(method	(doit theComment
		...
		(if
			...
			(= fd (FOpen (Format @str SAVE 0 (gGame name?))))
			(if (== fd -1)
			...
			(cond
				((== i changeDirI)
					(if (GetDirectory curSaveDir)
						(= numGames
							(GetSaveFiles (gGame name?) @names @nums)
						)
						(if (== numGames -1)
							(= ret -1)
							(break)
